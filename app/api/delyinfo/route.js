...
export async function PUT(request) {
    try {
        ...
        const { _id, delname, delconno, deltime } = requestBody;

        if (!_id || !delname || !delconno || deltime) {
            return new Response(
                JSON.stringify({ error: "delname, delconno, and _id are required" }),
                { status: 400 }
            );
        }
